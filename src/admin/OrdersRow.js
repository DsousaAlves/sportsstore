...
export class OrdersRow extends Component {
    ...
    calcTotal = (products) => products.reduce((total,p) => total += p.quantity * ( p.product !== null ? p.product.price : 0), 0).toFixed(2)

    getShipping = (order) => order.shipped ? <i className="fa fa-shipping-fast text-success"></i> : <i className="fa fa-exclamation-circle text-danger"></i>
